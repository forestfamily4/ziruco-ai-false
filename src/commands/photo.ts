...
export async function exec(message: Message, _args: string[], arg: string) {
  ...
  while(true) {
    ...
    const messages: Collection<string,Message> = await channel.messages.fetch({ limit: 100, after: afterChannelId })
    if (messages.size === 0) {
      break;
    }
    ...
    messages.forEach(m => {
      ...
      const regex = /(https?:\/\/[^\s]*\.(?:png|jpg|jpeg|gif|mp4|webm|mov|avi|wmv|flv|mkv)(?:\?[^\s]*)?)/ig;
...
async function download(photo: Photo){
  ...
  const ext=photo.url.split("/").pop()?.split("?")[0]?.split(".").pop()??"jpg";
